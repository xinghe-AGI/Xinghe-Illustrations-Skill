...
2. 视觉路由：再读 `visual-routing-and-candidates.md`，决定 `primary_route`、`secondary_routes`、信息密度、候选数量、图型和人物呈现等级。
3. 视觉形态：按任务读取 `visual-formats.md`、`technical-architecture-and-flow.md`、`composition-patterns.md` 或平台封面相关文件。
4. 统一风格：所有任务都以 `style-dna.md` 为最终视觉口径；冷白/浅蓝灰背景、手绘结构分区、文字颜色层级优先在这里统一。
5. 提示词与生成：读取 `prompt-template.md`、`prompt-template-images-api.md`、`reference-images.md`、`access-modes.md` 和 `image-generation-runtime.md`。
6. 验收返工：读取 `qa-checklist.md`；封面任务再读 `cover-qa-checklist.md`。
