...
description: 生成“星禾”个人 IP 风格的中文内容视觉资产：文章正文配图、微信公众号封面、小红书封面、情绪锚点图、解释图、多格漫画、知识卡片组、单张知识卡片、信息图海报、全景信息图、技术架构图和流程图。用于公众号文章、Markdown、本地长文、方法论、项目复盘、技术说明、工作流说明、教程、产品介绍或社媒内容的视觉路由、候选方向、prompt-only、manifest 和 PNG 生成。含人物时必须传入 `assets/examples/00-xinghe-ip-baseline.png` 人物基准图；结构主导图可让人物变小、局部出现或不出现。
---
...
这是星禾统一内容生图 skill。它不再只负责 16:9 正文插图，而是负责把一篇中文内容路由成适合发布和阅读的视觉表达：正文配图、平台封面、情绪锚点图、解释图、多格漫画、知识卡片组、单张知识卡片、信息图海报、全景信息图、技术架构图和流程图。
...
- `panoramic-infographic`：横向高密度全景图，用于系统地图、能力全景、方法全貌、架构总览、输入-核心循环-输出-对比-工具箱一屏讲清；默认最多 1 张，必要时拆成知识卡片组。
...
- `panoramic-infographic + knowledge-card-pack`：内容像系统全景、产品能力图、方法论地图或工程路径时，先给 1 张横向全景图说明整体关系，再用卡片组拆解每个模块。
...
- 全景信息图：用于横向一屏讲清系统、方法、架构或全链路。默认 `16:9` 或更宽横版，允许 7-10 个信息区和更高标签密度，但必须有主阅读路径、中央主链路、左右输入输出或侧栏解释；人物默认 `small-character`、`partial-character` 或 `no-character`。
...
- 横向流程、时间线、多模块对照、桌面阅读、公众号正文横向配图型知识卡、全景信息图：优先 `16:9`。
- 技术架构图、流程图：优先 `4:3` 或 `16:9`；移动端知识卡片化时可拆成多张 `3:4`。
- 全景信息图如果用于桌面阅读、文章内嵌或方法全貌，优先 `16:9`；如果模块过多，拆成“全景图 + 3-7 张知识卡片”，不要改成超小字长图。
