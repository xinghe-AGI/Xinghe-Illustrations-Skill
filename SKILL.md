...
description: 生成“星禾”个人 IP 风格的中文配图、微信公众号文章封面、小红书笔记封面、文章头图和社媒首图，统一面向 Codex、OpenClaw、Hermes 和其他支持本地 skills 的环境。用于社媒内容、账号运营、AI 自动化、工作流 SOP、内容日历、选题生产、发布复盘、私域承接、个人品牌、产品策略、方法论、研究笔记和观点表达；可输出配图策略、shot list、单张生图提示词、改图提示，并在 API key/endpoint 可用且用户授权外部上传风险时，通过内置 Node CLI 调用 Responses 或 Images API，支持 official/proxy。真实生图必须通过 `--style-references` 传入 `assets/examples/00-xinghe-ip-baseline.png` 人物基准图；如果当前原生生图工具不能传入参考图，不得只靠文字 prompt 声称生成合格星禾图；调用 official/proxy/API 前必须确认用户确实授权真实生图及外部数据上传风险；遇到“公众号封面”“微信文章头图”“小红书封面”“笔记首图”“生成封面图”“生成正文配图”等请求时使用。
---
...
本 skill 合并了原 `xinghe-illustrations-skill` 与 `xinghe-illustrations-skill-codex` 的规则，统一保留 `xinghe-illustrations-skill` 目录和 `xinghe-AGI/Xinghe-Illustrations-Skill` 仓库。Codex 原生 `image_gen` 如果只能接收文字 prompt，不能把 `assets/examples/00-xinghe-ip-baseline.png` 作为真实图片参考输入，就不要用它真实生成星禾图；默认输出 prompt-only、shot list 或候选方向。只有用户明确授权使用外部 CLI/API，并理解文章提示词、本地人物基准图和场景参考图可能被发送到官方 OpenAI 或第三方中转服务后，才可以调用本 skill 的 Node CLI。
## 安装后配置提醒
...
真实需要生成图片且环境可用、并且用户已经授权外部数据上传风险时，使用内置 CLI。Codex 原生 `image_gen` 当前若不能传入参考图片，只能作为 prompt-only 辅助，不用于合格星禾图的真实生成。所有真实生成命令必须传入人物基准图。先选择 1 张场景参考图，并在 skill 目录运行：
...
- 不在 Codex 原生 `image_gen` 不能传参考图时，静默切换到 CLI/proxy/API；必须先解释外部上传风险并获得明确授权。
